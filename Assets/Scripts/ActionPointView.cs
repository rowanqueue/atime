...
public class ActionPointView : MonoBehaviour
{
    ...
    /// </summary>
    /// <returns></returns>
    private IEnumerator OrbBounceRoutine()
    {
        float timer = 0;
        bool goingUp = true;
        ...
        while (true)
        {
            ...
            if (goingUp)
            {
                orbSprite.transform.localPosition = Vector3.Lerp(orgPos, newPos, Services.Visuals.GetEasedValue(EasingStyle.EaseInEaseOut, timer / bounceRate));
            }
            else
            {
                orbSprite.transform.localPosition = Vector3.Lerp(newPos, orgPos, Services.Visuals.GetEasedValue(EasingStyle.EaseInEaseOut, timer / bounceRate));
            }
